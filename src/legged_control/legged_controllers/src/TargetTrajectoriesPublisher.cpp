...
int main(int argc, char** argv) {
  ...
  ::ros::NodeHandle nh;       // 受 <group ns="dog1"> 影響 → namespace = /dog1/
  ::ros::NodeHandle pnh("~"); // private namespace → /dog1/legged_robot_target/

  // === 從 private 參數讀 robotName ===
  ...
  std::string taskFile;
  nh.getParam("referenceFile", referenceFile);  // 讀 /dog1/referenceFile
  nh.getParam("taskFile", taskFile);             // 讀 /dog1/taskFile
  ...
  TargetTrajectoriesPublisher target_pose_command(nh, robotName, &goalToTargetTrajectories, &cmdVelToTargetTrajectories);
