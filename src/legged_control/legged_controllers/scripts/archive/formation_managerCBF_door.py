...
class LeaderNavigator:
    ...
    def force_via_waypoint(self, start, via, reason=""):
        """
        Deadlock recovery: keep the original goal but replace the current path
        with start → via → original goal. This avoids re-generating the same
        stuck path when the robot is pressed against a door corner.
        """
        with self._lock:
        ...
        second = self._astar.plan(safe_via, safe_goal)
        ...
        with self._lock:
            if self._mode == self.MODE_AUTO and self._goal == goal:
                ...
                rospy.logwarn(
                    "[LeaderNavigator] recovery via waypoint (%.2f, %.2f): %s",
...
# ═══════════════════════════════════════════════════════════════
# Module C.6: DoorPassageManager
# ═══════════════════════════════════════════════════════════════

class DoorPassageManager:
    """
    A narrow-door formation state machine.

    Normal mode keeps the regular formation target. Near the doorway, followers
    switch to queue targets and pass one by one, then the manager releases them
    back to the normal formation once dog3 clears the door.
    """
    ...
    def __init__(self, follower_names, door_x=6.0, door_y=0.0,
                 detect_x_before=1.4, detect_x_after=1.2, detect_y=1.6,
                 leader_pass_x=6.55, follower_pass_x=6.45,
                 queue_leader_gap=1.15, queue_dog_gap=1.05,
                 pass_y=0.28, side_clear_y=0.85):
        self.enabled = True
        self.follower_names = list(follower_names)
        self.door_x = float(door_x)
        self.door_y = float(door_y)
        self.detect_x_before = float(detect_x_before)
        self.detect_x_after = float(detect_x_after)
        self.detect_y = float(detect_y)
        self.leader_pass_x = float(leader_pass_x)
        self.follower_pass_x = float(follower_pass_x)
        self.queue_leader_gap = float(queue_leader_gap)
        self.queue_dog_gap = float(queue_dog_gap)
        self.pass_y = float(pass_y)
        self.side_clear_y = float(side_clear_y)
        self.leader_pass_offset = abs(self.leader_pass_x - self.door_x)
        self.follower_pass_offset = abs(self.follower_pass_x - self.door_x)
        self.direction = 0  # +1: x<door -> x>door, -1: x>door -> x<door
    ...
    def _near_door(self, leader):
        return (
            self.door_x - self.detect_x_before <= leader.x <= self.door_x + self.detect_x_after
            and abs(leader.y - self.door_y) <= self.detect_y
        )

    def _side(self, x, deadband=0.15):
        if x < self.door_x - deadband:
            return -1
        if x > self.door_x + deadband:
            return 1
        return 0

    def _crossing_direction(self, leader, goal):
        if goal is None:
            return 0
        leader_side = self._side(leader.x)
        goal_side = self._side(goal[0])
        if leader_side == 0 or goal_side == 0 or leader_side == goal_side:
            return 0
        return goal_side

    def _passed(self, state, offset):
        return self.direction * (state.x - self.door_x) > offset

    def _queue_x(self, leader):
        if self.direction > 0:
            dog2_x = min(self.door_x - 0.85, leader.x - self.queue_leader_gap)
            dog3_x = min(self.door_x - 1.75, dog2_x - self.queue_dog_gap)
        else:
            dog2_x = max(self.door_x + 0.85, leader.x + self.queue_leader_gap)
            dog3_x = max(self.door_x + 1.75, dog2_x + self.queue_dog_gap)
        return dog2_x, dog3_x

    def update(self, states, leader_name, goal=None):
        if not self.enabled or len(self.follower_names) < 2:
            return {}

        leader = states[leader_name]
        dog2 = states[self.follower_names[0]]
        dog3 = states[self.follower_names[1]]
        if not leader.received or not dog2.received or not dog3.received:
            return {}

        prev = self.state
        if self.state == self.IDLE:
            direction = self._crossing_direction(leader, goal)
            if direction != 0 and self._near_door(leader):
                self.direction = direction
                self.state = self.QUEUE
        elif self.state == self.QUEUE:
            if self._passed(leader, self.leader_pass_offset):
                self.state = self.DOG2_PASS
            elif not self._near_door(leader):
                self.direction = 0
                self.state = self.IDLE
        elif self.state == self.DOG2_PASS:
            if self._passed(dog2, self.follower_pass_offset) and abs(dog2.y - self.door_y) < 0.55:
                self.state = self.DOG3_PASS
        elif self.state == self.DOG3_PASS:
            if self._passed(dog3, self.follower_pass_offset) and abs(dog3.y - self.door_y) < 0.55:
                self.direction = 0
                self.state = self.IDLE

        if self.state != prev:
            rospy.logwarn("[DoorPassage] %s → %s (dir=%+d)", prev, self.state, self.direction)

        if self.state == self.IDLE:
            return {}

        dog2_name, dog3_name = self.follower_names[0], self.follower_names[1]
        yaw = leader.yaw
        if self.state == self.QUEUE:
            dog2_x, dog3_x = self._queue_x(leader)
            return {
                dog2_name: (dog2_x, self.door_y + 0.12, yaw),
                dog3_name: (dog3_x, self.door_y - 0.12, yaw),
            }
        if self.state == self.DOG2_PASS:
            return {
                dog2_name: (self.door_x + self.direction * 0.75, self.door_y + self.pass_y, yaw),
                dog3_name: (self.door_x - self.direction * 0.85, self.door_y - 0.12, yaw),
            }
        if self.state == self.DOG3_PASS:
            return {
                dog2_name: (self.door_x + self.direction * 0.95, self.door_y + 1.30, yaw),
                dog3_name: (self.door_x + self.direction * 0.75, self.door_y - self.pass_y, yaw),
            }
        return {}

    def recovery_waypoint(self, leader):
        if self.direction != 0:
            return (self.door_x + self.direction * 0.8, self.door_y)
        if leader.x < self.door_x - 0.15:
            return (self.door_x - 0.75, self.door_y)
        if leader.x < self.door_x + 0.55:
            return (self.door_x + 0.8, self.door_y)
        return (max(self.door_x + 0.8, leader.x - 0.45), self.door_y)
...
class FleetManagerCBF:
    ...
    def __init__(self):
        ...
        # ── Module C.6: DoorPassageManager ──
        self.door_manager = DoorPassageManager(
            self.follower_names,
            door_x=rospy.get_param("~door_x", _CFG.get("door_x", 6.0)),
            door_y=rospy.get_param("~door_y", _CFG.get("door_y", 0.0)),
            detect_x_before=rospy.get_param(
                "~door_detect_x_before", _CFG.get("door_detect_x_before", 1.4)),
            detect_x_after=rospy.get_param(
                "~door_detect_x_after", _CFG.get("door_detect_x_after", 1.2)),
            detect_y=rospy.get_param("~door_detect_y", _CFG.get("door_detect_y", 1.6)),
            leader_pass_x=rospy.get_param(
                "~door_leader_pass_x", _CFG.get("door_leader_pass_x", 6.55)),
            follower_pass_x=rospy.get_param(
                "~door_follower_pass_x", _CFG.get("door_follower_pass_x", 6.45)),
            queue_leader_gap=rospy.get_param(
                "~door_queue_leader_gap", _CFG.get("door_queue_leader_gap", 1.15)),
            queue_dog_gap=rospy.get_param(
                "~door_queue_dog_gap", _CFG.get("door_queue_dog_gap", 1.05)),
            pass_y=rospy.get_param(
                "~door_pass_y", _CFG.get("door_pass_y", 0.28)),
            side_clear_y=rospy.get_param(
                "~door_side_clear_y", _CFG.get("door_side_clear_y", 0.85)),
        )
        self.door_manager.enabled = rospy.get_param(
        ...
        rospy.loginfo("  door_mode   = %s at (%.2f, %.2f)",
                      self.door_manager.enabled,
                      self.door_manager.door_x,
                      self.door_manager.door_y)
    ...
    def spin(self): # 主迴圈，每個 cycle 做四件事 1. 等待leader 2. 取得 leader 的 nominal 速度  3. 取得所有狗的 nominal 速度 4. CBF過濾
        ...
        while not rospy.is_shutdown():
            ...
            door_targets = self.door_manager.update(
                states, self.leader_name, self.navigator.current_goal)
            ...
            for name in self.follower_names:
                if self.followers_stationary:
                    ...
                    if name in door_targets:
                        x_ref, y_ref, yaw_ref = door_targets[name]
                    else:
            ...
            self._update_stuck_state(u_safe[self.leader_name], states)

            rate.sleep()

    def _update_stuck_state(self, leader_u_safe, states):
        """
        ...
        via = self.door_manager.recovery_waypoint(leader)
        if self.door_manager.enabled and self.navigator.force_via_waypoint(tuple(leader.pos), via, reason):
            self._consec_replans += 1
            self._stuck_counter = 0
            self._cooldown_counter = self.stuck_replan_cooldown
        elif self.navigator.force_replan(reason):
            self._consec_replans += 1
            self._stuck_counter = 0
            self._cooldown_counter = self.stuck_replan_cooldown
